    ...
    vector<function<void()>> lambdaFunctions = {
            ...
            [&]() {
                ...
                const char* sql =
                        ...
                        //Our idempotent form (IF NOT EXISTS / OR IGNORE / upsert) is kept - it
                        //lets a partially-created database be re-opened instead of erroring -
                        //while adopting upstream's v7 schema: the new ddSyncHeight flag and the
                        //bumped dbVersion.
                        ...
                        "INSERT INTO \"flags\"(\"key\",\"value\") VALUES (\"dbVersion\",7) ON CONFLICT(\"key\") DO UPDATE SET \"value\"=7;"
...
void Database::reset() {
    ...
    const char* sql = "BEGIN;"
                      ...
                      //DigiDollar tables clear with everything else, and ddSyncHeight resets so
                      //the next start re-runs the activation backfill (upstream 1b06828). These
                      //go INSIDE our transaction - reset() runs on deep-reorg recovery and a
                      //failure part-way must not leave the DB half-wiped.
                      "DELETE FROM ddutxos;"
